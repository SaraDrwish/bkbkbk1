        ...
        const newArticle = new Article({ title, category, publishDate, content, isPublished });
         ...
         if (req.files && req.files.length > 0) {
            const imageFileNames = await Promise.all(
                req.files.map(async (file) => await fileUploadService.uploadFile(file))
            );
            newArticle.images = imageFileNames;
        }
         ...
         if (req.files && req.files.length > 0) {
            const imageFileNames = await Promise.all(
             ...
             if (article.images && article.images.length > 0) {
                await Promise.all(
                    article.images.map(async (imageName) => await fileUploadService.deleteFile(imageName))
                );
            }

             article.images = imageFileNames;
        }
